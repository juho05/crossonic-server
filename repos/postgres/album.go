...
func genAlbumJoins(include repos.IncludeAlbumInfo) *bqb.Query {
	...
	if include.PlayInfo && include.User != "" {
		q.Space(`LEFT JOIN (
			SELECT album_id, COUNT(*) as count, MAX(time) as last_played FROM scrobbles WHERE user_name = ? AND album_id IS NOT NULL AND now_playing = false AND (duration_ms IS NULL OR duration_ms >= 240000 OR duration_ms >= song_duration_ms*0.5) GROUP BY (user_name, album_id)
...
func getAlbumArtistRefs(ctx context.Context, db executer, albumIDs []string) (map[string][]repos.ArtistRef, error) {
	q := bqb.New(`SELECT album_artist.album_id, artists.id, artists.name, artists.music_brainz_id FROM album_artist
		...
		WHERE album_artist.album_id IN (?) ORDER BY album_artist.index`, albumIDs)
